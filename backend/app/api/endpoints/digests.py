...
def get_latest_digest(
    ...
    if not digest:
        # Create a new digest if none exists
        curation_service = CurationService(db)
        digest = curation_service.create_digest(edition)
...
@router.post("/create/{edition}")
def create_digest_manual(
    edition: str,
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Manually trigger digest creation (admin only)"""
    ...
    return {
        "message": f"Digest created successfully",
        "digest_id": digest.id,
